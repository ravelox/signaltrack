...
export class PostgresReportStore implements ReportStore {
  ...
  public async create(input: CreateReportInput): Promise<ReportRecord> {
    const [row] = await this.db<ReportRecord[]>`
      ...
      VALUES (${input.orgId}::uuid, ${input.reporterUserId}::uuid, ${input.reporterType}, ${input.rawDescription}, ${input.expectedBehavior ?? null}, ${input.observedBehavior ?? null}, ${input.impactLevel}, ${input.workaroundAvailable ?? null}, ${input.contactAllowed}, ${JSON.stringify(input.environmentSnapshot)}::jsonb)
